"""
===============================================================================
MLAI CANDLE LANGUAGE ENGINE V1.1
===============================================================================
...
V1.1 fixes:
    - Correctly displays OPEN price.
    - Correctly displays HIGH price.
    - Correctly displays LOW price.
    - Correctly displays CLOSE price.
    - Verifies candle OHLC geometry.
    - Attempts to read KB source hash from multiple possible KB layouts.
    - Explicitly verifies that KB belongs to the current market_data.bin.
    - Keeps market_data.bin and KB strictly read-only.
===============================================================================
"""
...
def validate_candle(candle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if h < max(o, c):
        raise ValueError(
            "Invalid candle: high is below open/close."
        )

    if l > min(o, c):
        raise ValueError(
            "Invalid candle: low is above open/close."
        )
...
def body_language(
    ...
    if body_ratio < 0.45:
        ...
        return (
            ...
            "This represents meaningful but not dominant directional movement."
        )
    ...
    if body_ratio < 0.75:
        ...
        return (
            ...
            f"{pct(body_ratio)} of its total range. "
            "Directional movement dominated much of the candle."
        )

    return (
        f"The candle has a very large body occupying approximately "
        f"{pct(body_ratio)} of its range. "
        "Most of the candle's movement occurred in the direction of the close."
    )
...
def wick_language(
    ...
    # FULL BODY
    if (
        upper_wick <= candle_range * 0.03
        and lower_wick <= candle_range * 0.03
    ):
...
def sequence_language(
    ...
    if len(movements) >= 4:
        ...
        first_half = movements[
            :len(movements) // 2
        ]

        second_half = movements[
            len(movements) // 2:
        ]
...
def translate_candle(
    ...
    if anatomy["direction_numeric"] > 0:
        direction = "bullish"

    elif anatomy["direction_numeric"] < 0:
        direction = "bearish"
...
def print_translation(
    ...
    # FIXED:
    # These values now come directly from the validated candle anatomy.

    print(
        f"Open          : {anatomy['open']:.6f}"
    )

    print(
        f"High          : {anatomy['high']:.6f}"
    )

    print(
        f"Low           : {anatomy['low']:.6f}"
    )

    print(
        f"Close         : {anatomy['close']:.6f}"
    )
    ...
    if human["wicks"]:
        ...
        print(
            "• No dominant wick behaviour detected."
        )
...
def main() -> None:
    ...
    print("MLAI CANDLE LANGUAGE ENGINE V1.1")
    ...
    print(
        "Loading candle-language knowledge base..."
    )
    ...
    if not isinstance(kb, dict):
        ...
        raise TypeError(
            "MLAI_CANDLE_LANGUAGE_KB_V1.bin does not contain "
    ...
    kb_source_hash = get_kb_source_hash(kb)
    ...
    # -------------------------------------------------------------------------
    # KB / MARKET DATA PROVENANCE CHECK
    # -------------------------------------------------------------------------

    print()

    print("KB / MARKET DATA PROVENANCE")

    if kb_source_hash:

        if kb_source_hash == market_hash:
            ...
            print(
                "Meaning               : KB was built from this exact "
                "market_data.bin byte content."
            )
            ...
            print(
                "Meaning               : KB source hash does not match "
                "the current market_data.bin."
            )

            print(
                "WARNING               : Do not treat this KB as synchronized "
                "with the current market data."
            )

    else:

        print(
            "Source hash match     : NOT AVAILABLE"
        )

        print(
            "Meaning               : KB does not expose its source hash "
            "in a recognized metadata field."
        )

    # -------------------------------------------------------------------------
    # ANALYZE ONLY LAST COMPLETED CANDLE
    # -------------------------------------------------------------------------
    ...
    previous_candles = candles[
        max(
            0,
            target_index - REFERENCE_CANDLE_COUNT
        ):target_index
    ]
    ...
    print(
        "Completed candle analyzed : YES"
    )

    print(
        "Future candles used       : NO"
    )
    ...
    print(
        "Knowledge base modified   : NO"
    )

    print(
        "BUY/SELL signal generated : NO"
    )

    print(
        "Hidden-order inference    : NO"
    )

    print(
        "Human-language translation: YES"
    )

    print(
        "Raw OHLC displayed        : YES"
    )

    print(
        "Candle geometry validated : YES"
    )
    ...
    print(
        "MLAI CANDLE LANGUAGE ENGINE V1.1 COMPLETE"
    )
