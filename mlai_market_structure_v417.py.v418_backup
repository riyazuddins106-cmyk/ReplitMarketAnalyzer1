...
# =============================================================================
# MLAI v4.1.7 VALIDATED BUILD
# =============================================================================
# Built from mlai_market_structure_v415.py after AST validation.
# The v4.1.7 retrieval architecture includes:
...
# =============================================================================
"""
MLAI v4.1.7 — ROBUST CAUSAL HISTORICAL EXPERIENCE RETRIEVAL
WITH DISCRIMINATION + INCREMENTAL PREDICTIVE VALUE

Research / validation only.

V4.1.7 preserves the V4.1.5 causal historical-experience architecture
and adds seven explicitly validated capabilities:
...
Important:
- V4.1.5 is not modified.
- market_data.bin is READ ONLY.
- No OOS outcome is supplied to the predictor.
- All retrieval candidates must satisfy the temporal barrier.
- H4/H8/H16 are evaluated independently.
- No OOS tuning.
- No live learning.
- No trading.

This is a research / validation phase.
"""
...
VALIDATION_BIN = "MLAI_V417_ROBUST_HISTORICAL_EXPERIENCE_RETRIEVAL.bin"
VALIDATION_REPORT = "MLAI_V417_ROBUST_HISTORICAL_EXPERIENCE_RETRIEVAL_REPORT.md"


# =====================================================================
# CORE CONFIGURATION — PRESERVED FROM V4.1.5
# =====================================================================
...
# =====================================================================
# V4.1.5 FIXED RETRIEVAL WEIGHTS
# =====================================================================
...
# =====================================================================
# V4.1.7 DISCRIMINATION CONFIGURATION
# =====================================================================

# Similarity representation:
#
# The original V4.1.5 total is preserved.
# V4.1.7 additionally exposes the full component representation.
#
# These are diagnostics, not OOS-tuned parameters.

DISCRIMINATION_MIN_MATCHES = 8

# Minimum similarity gap required for a query to be considered
# meaningfully discriminative relative to its candidate population.
MIN_SIMILARITY_SEPARATION = 0.05

# V4.1.7: discrimination must be measured against the complete
# eligible candidate population, not merely against the selected
# neighbours.  The old test could classify essentially every query
# as discriminative because top-neighbour similarity was naturally
# higher than the population average.
DISCRIMINATION_TOP_FRACTION = 0.10
DISCRIMINATION_MIN_EFFECT = 0.02
DISCRIMINATION_MIN_DIRECTIONAL_EFFECT = 0.02
DISCRIMINATION_NULL_PERMUTATIONS = 25

# Prediction integration thresholds.
PREDICTION_MIN_PROBABILITY = 0.40
PREDICTION_MIN_MARGIN = 0.05

# Incremental value thresholds are diagnostic only.
#
# Positive values mean the retrieval/predictive layer improved over
# the appropriate baseline.
INCREMENTAL_VALUE_EPS = 1e-12
...
def entropy3(
    ...
    maximum = math.log(3.0)

    return safe_div(result, maximum)
    ...
    for state in states:
        ...
        if state.last_high_index is not None:
            ...
            if (
                ...
                reasons.append(
                    f"Future high visible at state {state.index}."
                )
        ...
        if state.last_low_index is not None:
            ...
            if (
                ...
                reasons.append(
                    f"Future low visible at state {state.index}."
                )
        ...
        if events.get(
            ...
            if (
                ...
                or state.event_index > state.index
            ):

                reasons.append(
                    f"Future event visible at state {state.index}."
                )
...
def build_path_vector(
    ...
    for current in range(
        ...
        current_atr = (
            atr[current]
            if atr[current] is not None
            else max(
                candles[current].high
                - candles[current].low,
                EPS,
            )
        )
        ...
        previous_close = (
            candles[current - 1].close
            if current > 0
            else candle.close
        )

        normalized_return = safe_div(
            candle.close
            - previous_close,
            current_atr,
        )

        normalized_range = safe_div(
            candle.high
            - candle.low,
            current_atr,
        )
...
def coarse_filter(
    ...
    for record in records:
        ...
        if (
            ...
            or record.structure_event
            == current.structure_event
            or record.sequence_state
...
def path_row_similarity(
    ...
    return_similarity = numeric_similarity(
        current_return,
        historical_return,
        1.0,
    )

    range_similarity = numeric_similarity(
        current_range,
        historical_range,
        1.0,
    )

    body_similarity = numeric_similarity(
        current_body,
        historical_body,
        1.0,
    )
...
def similarity_representation(
    ...
    structure = mean_or_zero(
        [
            ...
            1.0
            if current.structure_event
            == record.structure_event
            else 0.0,

            1.0
            if current.high_label
            == record.state_key[2]
            else 0.0,

            1.0
            if current.low_label
            == record.state_key[3]
            else 0.0,
        ]
    )
    ...
    momentum = (
        1.0
        if current.momentum_state
        == record.momentum_state
        else 0.0
    )

    volatility = numeric_similarity(
...
def similarity_score(
    ...
    representation = similarity_representation(
        current,
        record,
    )
    ...
    return {
        ...
        "momentum": representation.momentum,
        "volatility": representation.volatility,
        "candle": representation.candle,
        "path": representation.path,
    }


# =====================================================================
# EPISODE REPRESENTATIVES
# =====================================================================
...
# =====================================================================
# V4.1.7 RETRIEVAL DISCRIMINATION RESULT
# =====================================================================
...
def calculate_retrieval_discrimination(
    ...
    """
    V4.1.7 STRICT RETRIEVAL DISCRIMINATION

    The previous V4.1.7 diagnostic could report discrimination for almost
    every query because it effectively rewarded the fact that retrieved
    neighbours are more similar to the query than the average candidate.

    That is not sufficient.

    V4.1.7 requires ALL of the following:

      1. enough eligible candidates;
      2. a genuine top-vs-remainder similarity effect;
      3. meaningful concentration of similarity in the selected set;
      4. directional concentration stronger than the candidate baseline;
      5. deterministic label-permutation sanity testing.

    This is diagnostic only. It never uses OOS outcomes to select
    candidates and never changes retrieval weights.
    """
    ...
    def _directional_concentration(records):
        if not records:
            return 0.0
    ...
    if (
        len(selected) < DISCRIMINATION_MIN_MATCHES
        or len(candidates) < DISCRIMINATION_MIN_MATCHES * 2
        or len(selected_records) < DISCRIMINATION_MIN_MATCHES
        or len(candidate_records) < DISCRIMINATION_MIN_MATCHES * 2
    ):
        return DiscriminationResult(
            ...
            mean_similarity_separation=0.0,
            mean_ranking_concentration=0.0,
            mean_directional_discrimination=0.0,
            mean_class_entropy=_entropy(
                [r.outcome.direction for r in selected_records]
            ),
            mean_baseline_entropy=_entropy(
                [r.outcome.direction for r in candidate_records]
            ),
            predictive_accuracy=0.0,
            retrieval_accuracy=0.0,
            baseline_accuracy=0.0,
            incremental_brier_lift=0.0,
            incremental_log_loss_lift=0.0,
            incremental_accuracy_delta=0.0,
            predictive_margin=0.0,
        )

    # ---------------------------------------------------------------
    # Similarities from the actual candidate population.
    #
    # This is the key correction: the selected neighbours are compared
    # with the remainder of the eligible population, not with a generic
    # or self-referential quantity.
    # ---------------------------------------------------------------

    similarities = [
        float(getattr(match, "similarity", 0.0))
    ...
    if top_n < DISCRIMINATION_MIN_MATCHES:
        top_n = DISCRIMINATION_MIN_MATCHES
    ...
    top_similarity = mean_or_zero(
        [value for _, value in top]
    )

    remainder_similarity = mean_or_zero(
        [value for _, value in remainder]
    )

    similarity_separation = (
        top_similarity
        - remainder_similarity
    )

    # ---------------------------------------------------------------
    # Ranking concentration.
    #
    # A useful retrieval system should put materially more similarity
    # mass in the top group than would be expected from its population
    # share alone.
    # ---------------------------------------------------------------
    ...
    top_similarity_mass = sum(
        value
        for _, value in top
    )

    if all_similarity > EPS:
        observed_share = (
            top_similarity_mass
            / all_similarity
        )
    ...
    #
    # Compare top-neighbour directional concentration against the full
    # eligible population's dominant directional concentration.
    # ---------------------------------------------------------------

    top_records = []

    selected_indices = {
        getattr(record, "index", None)
    ...
    if not top_records:
        top_records = selected_records[:top_n]

    top_directional = _directional_concentration(
        top_records
    )

    baseline_directional = _directional_concentration(
        candidate_records
    )

    directional_discrimination = max(
        0.0,
        top_directional
    ...
    # ---------------------------------------------------------------
    # Class entropy diagnostics.
    # ---------------------------------------------------------------

    class_entropy = _entropy(
        [
            record.outcome.direction
    ...
    # Strict decision.
    #
    # All conditions must pass. This prevents the old "100% by
    ...
    discriminative = (
        ...
        and ranking_concentration
        >= DISCRIMINATION_MIN_EFFECT
        and directional_discrimination
        >= DISCRIMINATION_MIN_DIRECTIONAL_EFFECT
    )

    return DiscriminationResult(
        queries=1,
        discriminative_queries=(
            1 if discriminative else 0
        ),
        discriminative=bool(discriminative),
        discrimination_rate=(
            1.0 if discriminative else 0.0
        ),
        mean_similarity_separation=similarity_separation,
        mean_ranking_concentration=ranking_concentration,
        mean_directional_discrimination=directional_discrimination,
        mean_class_entropy=class_entropy,
        mean_baseline_entropy=baseline_entropy,
        predictive_accuracy=0.0,
        retrieval_accuracy=0.0,
        baseline_accuracy=0.0,
        incremental_brier_lift=0.0,
...
def _serialize_discrimination_result(result):
    """
    Canonical V4.1.7 discrimination-result contract.

    The calculation layer may use aggregate/internal names such as
    mean_similarity_separation.  The reporting layer uses the public
    names similarity_separation, ranking_concentration, etc.

    This function exposes BOTH representations from one place so the
    calculation and reporting layers cannot drift apart.
    """
    ...
    # Canonical aliases used by the existing V4.1.7 reporting layer.
    alias_pairs = {
        "similarity_separation":
    ...
    for public_name, internal_name in alias_pairs.items():

        if public_name not in data:

            if internal_name in data:
                data[public_name] = data[internal_name]

            else:
                data[public_name] = 0.0

        if internal_name not in data:
            data[internal_name] = data[public_name]

    # The report expects a boolean discriminative decision.
    if "discriminative" not in data:

        data["discriminative"] = (
            int(data.get("discriminative_queries", 0)) > 0
        )

    # Guarantee all incremental/predictive fields exist.
    defaults = {
        "queries": 0,
        "discriminative_queries": 0,
        "discrimination_rate": 0.0,
...
def retrieve_historical_experience(
    ...
    selected = select_episode_representatives(
        candidates
    )
    ...
    discrimination = calculate_retrieval_discrimination(
        selected,
        candidates,
        selected_records,
        candidate_records,
    ...
    top_similarity = selected[
        0
    ].similarity

    mean_similarity = mean_or_zero(
        [
            match.similarity
            for match, _
            in selected_rows
    ...
    structure_agreement = mean_or_zero(
        [
...
def main() -> None:
    ...
    print(
        f"Confirmed swings          : {len(engine.swings)}"
    )
    ...
    print(
        f"Episodes                  : {len(set(episode_ids.values()))}"
    )
    ...
    for window in windows:
        ...
        for horizon in HORIZONS:
                ...
                "log_loss_lift":
                    (
                        ...
                        if (
                            retrieval_log_loss
                            is not None
                            and baseline_log_loss
                            is not None
                        )
                        else None
                    ),

                # V4.1.7 predictive integration
                "predictive_accuracy":
                    predictive_accuracy,
                ...
                "predictive_log_loss_lift":
                    (
                        ...
                        if (
                            baseline_log_loss
                            is not None
                            and predictive_log_loss
                            is not None
                        )
                        else None
                    ),

                # V4.1.7 discrimination
                "discrimination":
                    discrimination_summary,
            ...
            print(
                f"H+{horizon}: "
                f"TrainRecords={len(records)} | "
                f"OOSQueries={len(evaluations)}"
            )

            print(
                f"Retrieval Accuracy = "
                f"{fmt_pct(retrieval_accuracy)} | "
                f"Baseline = "
                f"{fmt_pct(baseline_accuracy)}"
            )

            print(
                f"Predictive Accuracy = "
                f"{fmt_pct(predictive_accuracy)}"
            )

            print(
                f"Retrieval Brier = "
                f"{fmt_num(retrieval_brier)} | "
                f"Predictive Brier = "
                f"{fmt_num(predictive_brier)} | "
                f"Baseline Brier = "
                f"{fmt_num(baseline_brier)}"
            )

            print(
                f"Predictive Brier Lift = "
                f"{fmt_num(result['predictive_brier_lift'])}"
            )

            print(
                f"Predictive LogLoss Lift = "
                f"{fmt_num(result['predictive_log_loss_lift'])}"
            )

            print(
                f"Discrimination Rate = "
                f"{fmt_pct(discrimination_summary['discrimination_rate'])}"
            )

            print(
                f"Similarity Separation = "
                f"{fmt_num(discrimination_summary['mean_similarity_separation'])}"
            )

            print(
                f"Directional Discrimination = "
                f"{fmt_num(discrimination_summary['mean_directional_discrimination'])}"
            )

            print(
                f"Predictive Margin = "
                f"{fmt_num(discrimination_summary['mean_predictive_margin'])}"
            )

            print(
                f"Coverage = "
                f"{fmt_pct(result['retrieval_coverage'])} | "
                f"Sparse = "
                f"{fmt_pct(result['sparse_rate'])}"
            )
    ...
    artifact = {
        ...
        "objective":
            "ROBUST_CAUSAL_HISTORICAL_EXPERIENCE_RETRIEVAL_WITH_DISCRIMINATION",
        ...
        "v416_capabilities": {
            ...
            "retrieval_ranking_discrimination":
                True,
            ...
            "predictive_decision_integration":
                True,
        },
    ...
    report.append(
        "## V4.1.7 seven capability status"
    )
    ...
    for horizon in HORIZONS:
        ...
        report.append(
            f"- OOS queries: {d['queries']}"
        )

        report.append(
            "- Discriminative queries: "
            f"{d['discriminative_queries']}"
        )

        report.append(
            "- Discrimination rate: "
            f"{fmt_pct(d['discrimination_rate'])}"
        )

        report.append(
            "- Mean similarity separation: "
            f"{fmt_num(d['mean_similarity_separation'])}"
        )

        report.append(
            "- Mean ranking concentration: "
            f"{fmt_num(d['mean_ranking_concentration'])}"
        )

        report.append(
            "- Mean directional discrimination: "
            f"{fmt_num(d['mean_directional_discrimination'])}"
        )

        report.append(
            "- Mean class entropy: "
            f"{fmt_num(d['mean_class_entropy'])}"
        )

        report.append(
            "- Mean baseline entropy: "
            f"{fmt_num(d['mean_baseline_entropy'])}"
        )

        report.append(
            "- Predictive accuracy: "
            f"{fmt_pct(d['predictive_accuracy'])}"
        )

        report.append(
            "- Retrieval accuracy: "
            f"{fmt_pct(d['retrieval_accuracy'])}"
        )

        report.append(
            "- Baseline accuracy: "
            f"{fmt_pct(d['baseline_accuracy'])}"
        )

        report.append(
            "- Incremental Brier lift: "
            f"{fmt_num(d['incremental_brier_lift'])}"
        )

        report.append(
            "- Incremental log-loss lift: "
            f"{fmt_num(d['incremental_log_loss_lift'])}"
        )

        report.append(
            "- Incremental accuracy delta: "
            f"{fmt_num(d['incremental_accuracy_delta'])}"
        )
    ...
    for horizon, result in aggregate.items():
        ...
        report.append(
            "- Retrieval accuracy: "
            f"{fmt_pct(result['mean_retrieval_accuracy'])}"
        )

        report.append(
            "- Baseline accuracy: "
            f"{fmt_pct(result['mean_baseline_accuracy'])}"
        )

        report.append(
            "- Predictive accuracy: "
            f"{fmt_pct(result['mean_predictive_accuracy'])}"
        )

        report.append(
            "- Retrieval Brier: "
            f"{fmt_num(result['mean_retrieval_brier'])}"
        )

        report.append(
            "- Baseline Brier: "
            f"{fmt_num(result['mean_baseline_brier'])}"
        )

        report.append(
            "- Retrieval Brier lift: "
            f"{fmt_num(result['mean_brier_lift'])}"
        )

        report.append(
            "- Predictive Brier: "
            f"{fmt_num(result['mean_predictive_brier'])}"
        )

        report.append(
            "- Predictive Brier lift: "
            f"{fmt_num(result['mean_predictive_brier_lift'])}"
        )

        report.append(
            "- Retrieval log loss: "
            f"{fmt_num(result['mean_retrieval_log_loss'])}"
        )

        report.append(
            "- Baseline log loss: "
            f"{fmt_num(result['mean_baseline_log_loss'])}"
        )

        report.append(
            "- Retrieval log-loss lift: "
            f"{fmt_num(result['mean_log_loss_lift'])}"
        )

        report.append(
            "- Predictive log loss: "
            f"{fmt_num(result['mean_predictive_log_loss'])}"
        )

        report.append(
            "- Predictive log-loss lift: "
            f"{fmt_num(result['mean_predictive_log_loss_lift'])}"
        )
        ...
        report.append(
            "- Sparse rate: "
            f"{fmt_pct(result['mean_sparse_rate'])}"
        )

        report.append(
            "- Mean top similarity: "
            f"{fmt_pct(result['mean_top_similarity'])}"
        )

        report.append(
            "- Mean independent matches: "
            f"{result['mean_matches']:.2f}"
        )
    ...
    if bucket_summary:
        ...
        for bucket, result in bucket_summary.items():
            ...
            report.append(
                "- Retrieval accuracy: "
                f"{fmt_pct(result['retrieval_accuracy'])}"
            )

            report.append(
                "- Predictive accuracy: "
                f"{fmt_pct(result['predictive_accuracy'])}"
            )

            report.append(
                "- Retrieval Brier: "
                f"{fmt_num(result['retrieval_brier'])}"
            )

            report.append(
                "- Predictive Brier: "
                f"{fmt_num(result['predictive_brier'])}"
            )

            report.append(
                "- Baseline Brier: "
                f"{fmt_num(result['baseline_brier'])}"
            )

            report.append(
                "- Retrieval Brier lift: "
                f"{fmt_num(result['retrieval_brier_lift'])}"
            )
            ...
            report.append(
                ...
                f"{fmt_num(result['mean_similarity_separation'])}"
            )

            report.append(
                "- Directional discrimination: "
                f"{fmt_num(result['mean_directional_discrimination'])}"
            )
        ...
        report.append(
            "No similarity buckets contained evaluations."
        )
    ...
    report.append(
        f"- Queries: {null_summary['queries']}"
    )

    report.append(
        "- Mean real maximum share: "
        f"{fmt_pct(null_summary['mean_real_max_share'])}"
    )

    report.append(
        "- Mean null maximum share: "
        f"{fmt_pct(null_summary['mean_null_max_share'])}"
    )

    report.append(
        "- Mean null 95th percentile: "
        f"{fmt_pct(null_summary['mean_null_p95'])}"
    )

    report.append(
        "- Mean real-minus-null: "
        f"{fmt_pct(null_summary['mean_real_minus_null'])}"
    )
    ...
    report.append(
        "V4.1.7 explicitly tests whether historical experience "
        "retrieval produces a distinguishable neighbour set, whether "
        "that discrimination exists independently at H4, H8 and H16, "
        "and whether the integrated predictive decision provides "
        "incremental value over the conditional baseline."
    )

    report.append("")

    report.append(
        "Positive discrimination is not treated as proof of "
        "trading profitability. All results remain research / "
        "validation evidence."
    )
    ...
    print("=" * 100)
    print(
        "V4.1.7 RETRIEVAL + DISCRIMINATION "
        "PHASE COMPLETE"
    )
    print("=" * 100)
    ...
    for horizon, result in aggregate.items():
        ...
        print(
            "  Predictive Margin        : "
            f"{fmt_num(d['mean_predictive_margin'])}"
        )
    ...
    print("=" * 100)
    print(
        "MLAI v4.1.7 ROBUST HISTORICAL "
        "EXPERIENCE RETRIEVAL COMPLETE"
    )
    print("=" * 100)
